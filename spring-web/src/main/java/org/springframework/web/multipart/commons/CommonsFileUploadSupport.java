...
public abstract class CommonsFileUploadSupport {
	 ...
	 */
	// CommonsMultipartResolver子类实现----返回
	...
	protected FileUpload prepareFileUpload(@Nullable String encoding) {
		...
		// 当编码格式与FileUpload编码格式不符合时
		if (encoding != null && !encoding.equals(fileUpload.getHeaderEncoding())) {
			// 获取DiskFileItemFactory对象并创建ServletFileUpload对象
	...
	protected MultipartParsingResult parseFileItems(List<FileItem> fileItems, String encoding) {
		...
		for (FileItem fileItem : fileItems) {
			if (fileItem.isFormField()) {  // 该表单项是普通类型
				String value;
				// getContentType:当为表单时返回null,当为file类型时,获得上传文件的类型
				String partEncoding = determineEncoding(fileItem.getContentType(), encoding);
				try {
					// 获取表单元素,以partEncoding编码格式
				...
				// 返回表单标签name属性的值。如上例中<input type="text" name="column" />column。
	...
	protected static class MultipartParsingResult {
		...
		// 表单name = value 键值对对象Map
		private final Map<String, String[]> multipartParameters;

		// name = ContentType 键值对对象Map
		...
		// 获取multipartParameters对象
		public Map<String, String[]> getMultipartParameters() {
			return this.multipartParameters;
		}

		// 获取multipartParameterContentTypes对象
